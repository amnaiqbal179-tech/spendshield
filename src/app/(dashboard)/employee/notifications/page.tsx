...
import { Bell, CheckCircle2, XCircle, AlertCircle, CheckCheck } from "lucide-react";
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#7C5CFC] via-[#9B7BFC] to-[#4F33E6] p-6 sm:p-8 text-white shadow-md">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button 
            ...
            className="self-start sm:self-auto inline-flex items-center gap-2 bg-white/15 hover:bg-white/25 text-white text-xs font-semibold px-4 py-2 rounded-xl backdrop-blur-md transition-all border border-white/20"
          >
            Refresh Feed
          </button>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E7E9F0] p-6 space-y-3 shadow-2xs">
        {loading ? (
          ...
          <div className="py-12 text-center space-y-2">
            <div className="mx-auto w-12 h-12 rounded-full bg-[#F4F5F7] flex items-center justify-center text-[#98A2B3]">
              <Bell size={20} />
            </div>
            <p className="text-sm font-medium text-[#171A21]">No notifications found.</p>
            <p className="text-xs text-[#98A2B3]">You're all caught up! Updates from managers will appear here.</p>
          </div>
        ) : (
          notifications.map((n) => {
            const isApproved = n.title?.includes("Approved") || n.type?.includes("APPROVED");
            const isRejected = n.title?.includes("Rejected") || n.type?.includes("REJECTED");

            return (
              <div 
                key={n.id} 
                className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${
                  n.read ? 'bg-white border-[#E7E9F0]' : 'bg-[#FAFAFC] border-[#7C5CFC]/30 shadow-xs'
                }`}
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    {!n.read && (
                      <span className="inline-block h-2 w-2 rounded-full bg-[#7C5CFC]" title="Unread" />
                    )}
                  </div>
                  ...
                  <span className="text-[10px] text-[#98A2B3] mt-2 block font-medium">
                    {new Date(n.createdAt).toLocaleDateString()} at {new Date(n.createdAt).toLocaleTimeString([], { hour: '2-decorator' as any, minute: '2-digit' })}
                  </span>
                </div>
              </div>
            );
